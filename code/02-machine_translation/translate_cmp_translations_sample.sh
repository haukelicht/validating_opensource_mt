#!/bin/bash
#SBATCH --job-name=translate_cmp_translations_sample
#SBATCH -t 18:00:00
#SBATCH --gpus=1
...
# ~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~ #
# Lehmann et al. (2024) translated CMP corpus 
DATAFILE=cmp_translations_sample

message "Start translating Lehmann et al. (2024) translated CMP corpus"
cp -n "${DATAPATH}/${DATAFILE}.tsv" "${DATAPATH}/${DATAFILE}_translated.tsv"
for model in m2m_100_1.2B m2m_100_418M opus-mt; do
